import { MRT_Row, MRT_RowData, MRT_RowModel, MRT_RowSelectionState, MRT_Updater, MRT_VisibilityState } from "material-react-table";
...
export function useMrtMemoModeRowsSelectionWorkaround<TableData extends MRT_RowData>({
	...
	useEffect(() => {
		try {
			...
			const groupRows = rowsGroupedRowModel?.flatRows.filter(row => Number.isNaN(Number(row.id)));
			const groupRowsObj = groupRows
				? groupRows.reduce(
					(acc: Record<string, MRT_Row<TableData>>, curr: MRT_Row<TableData>) => ((acc[curr.id] = curr), acc),
					{}
				)
				: {};

			// Get the DOM table rows as an array
			const rowsDom = document.querySelectorAll<HTMLElement>(`#${gridId} tbody > tr`);
			...
			rowsDom.forEach((row, index, rows) => {
				...
				const rowCheckbox = row.querySelector<HTMLElement>('td > [data-row-id]');
				...
				if (rowCanExpand === 'true') {
					...
					if (index === rows.length - 1) {
						...
						const nextRow_1_Checkbox = nextRow_1?.querySelector<HTMLElement>('td > [data-row-id]');
						const nextRow_1_CanExpand = nextRow_1_Checkbox?.getAttribute('data-row-can-expand');

						if (nextRow_1 && nextRow_1_CanExpand && nextRow_1_CanExpand === 'true') {
							const nextRow_1_HasParent = nextRow_1_Checkbox?.getAttribute('data-row-has-parent');
						...
						if (nextRow_1_rowId) {
							...
							if (currentRowObj && currentRow_1_Obj && currentRow_1_Obj.depth > currentRowObj.depth) {
								isExpanded = true;
							}
						}
					}

					let expandIconSvg = row.querySelector<HTMLElement>('td > span > button[aria-label="Expand"] > svg');

					if (!expandIconSvg) {
						expandIconSvg = row.querySelector<HTMLElement>('td > span > button[aria-label="Collapse"] > svg');
					}

					if (expandIconSvg) {
						expandIconSvg.style.transform = `rotate(${isExpanded ? -180 : 0}deg)`;
					}
				}
			...
			groupRows?.forEach(groupRowData => {
				...
				const isRowLeafSelected = groupLeafRows.some(leafRow => leafRow.id in rowSelectionState);
				...
				const rowDomCheckbox = document.querySelector<HTMLElement>(rowData_DomSelector);
...
export const handleColumnVisibilityChangeGenerator = ({
	...
	return (updater: MRT_Updater<MRT_VisibilityState>) => {
		const newColumnVisibility: MRT_VisibilityState = updater instanceof Function ? updater(columnVisibility) : updater;
